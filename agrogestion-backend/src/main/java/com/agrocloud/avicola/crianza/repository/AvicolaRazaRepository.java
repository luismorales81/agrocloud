...
public interface AvicolaRazaRepository extends JpaRepository<AvicolaRaza, Long> {
    ...
    @Query("SELECT r FROM AvicolaRaza r WHERE r.empresaId = :empresaId AND r.moduloOrigen = :modulo ORDER BY r.nombre ASC")
    List<AvicolaRaza> listarPorEmpresaIdYModulo(
    ...
    @Query("SELECT r FROM AvicolaRaza r WHERE r.id = :id AND r.empresaId = :empresaId AND r.moduloOrigen = :modulo")
    Optional<AvicolaRaza> buscarPorIdYEmpresaIdYModulo(
