...
/**
 * Mapa y coordenadas de establecimientos de crianza (reutiliza el editor de huevos: mismo formato JSON).
 */
const UbicacionEstablecimientosCrianzaScreen: React.FC = () => {
  const navigate = useNavigate();
  const [searchParams, setSearchParams] = useSearchParams();

  const [lista, setLista] = useState<AvicolaEstablecimientoRespuesta[]>([]);
  ...
  const guardar = async () => {
    ...
    try {
      ...
      setError(null);
      setExito(null);
      await actualizarEstablecimientoCrianza(Number(seleccionId), {
  ...
  return (
    <Box sx={{ p: 3, maxWidth: 1100, mx: 'auto' }}>
      <Typography variant="h4" component="h1" gutterBottom>
        Ubicación en mapa — Crianza
      </Typography>
      <Typography variant="body2" color="text.secondary" paragraph>
        Elegí un establecimiento, dibujá el perímetro o un punto GPS y guardá. Misma idea que «Campos» en cultivos y que
        la pantalla equivalente en huevos.
      </Typography>
      ...
      {!cargando && (
        <Paper variant="outlined" sx={{ p: 2 }}>
          <FormControl fullWidth sx={{ mb: 2 }} size="small">
            <InputLabel id="sel-est-crianza">Establecimiento</InputLabel>
            <Select
              labelId="sel-est-crianza"
          ...
          {seleccionId !== '' && seleccionado && (
            <>
              ...
              <Box sx={{ mt: 2, display: 'flex', flexWrap: 'wrap', gap: 1 }}>
                ...
                <Button variant="outlined" onClick={() => navigate('/avicola-crianza/establecimientos')}>
...
export default UbicacionEstablecimientosCrianzaScreen;
