...
const RazasCrianzaScreen: React.FC = () => {
  ...
  const cargar = useCallback(async () => {
    try {
      setCargando(true);
      const r = await listarRazasCrianza();
      setRazas(r);
      setError(null);
    } catch (err: unknown) {
  ...
  const guardarRaza = async () => {
    try {
      ...
      if (razaEditando) {
        await actualizarRazaCrianza(razaEditando.id, { nombre: razaNombre.trim(), activo: razaActivo });
      } else {
        await crearRazaCrianza({ nombre: razaNombre.trim(), activo: razaActivo });
      }
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      <Typography variant="body2" color="text.secondary" paragraph>
        Razas o líneas usadas en los lotes de crianza. Los establecimientos tienen su propia entrada en el menú.
      </Typography>
...
export default RazasCrianzaScreen;
