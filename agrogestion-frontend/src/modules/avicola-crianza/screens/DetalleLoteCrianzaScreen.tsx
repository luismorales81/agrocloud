...
import {
  obtenerLoteCrianza,
  obtenerResumenCrianza,
  listarPesadasCrianza,
  registrarPesadaCrianza,
  listarMuertesCrianza,
  registrarMuerteCrianza,
  listarVentasCrianza,
  registrarVentaCrianza,
  listarConsumosCrianza,
...
const DetalleLoteCrianzaScreen: React.FC = () => {
  const { loteId: loteIdParam } = useParams<{ loteId: string }>();
  const navigate = useNavigate();
  const loteId = loteIdParam ? parseInt(loteIdParam, 10) : NaN;

  const [pestana, setPestana] = useState(0);
  const [lote, setLote] = useState<AvicolaLoteRespuesta | null>(null);
  const [resumen, setResumen] = useState<AvicolaCrianzaResumenRespuesta | null>(null);
  const [pesadas, setPesadas] = useState<AvicolaPesadaRespuesta[]>([]);
  const [muertes, setMuertes] = useState<AvicolaMuerteRespuesta[]>([]);
  const [ventas, setVentas] = useState<AvicolaVentaRespuesta[]>([]);
  const [consumos, setConsumos] = useState<AvicolaConsumoRespuesta[]>([]);
  const [eventos, setEventos] = useState<AvicolaEventoSanitarioRespuesta[]>([]);
  ...
  const [tipoVenta, setTipoVenta] = useState<TipoVentaCrianza>('VENTA_EN_PIE');
  ...
  const recargarListas = useCallback(async () => {
    ...
    const [p, m, v, c, e] = await Promise.all([
      listarPesadasCrianza(loteId),
      listarMuertesCrianza(loteId),
      listarVentasCrianza(loteId),
      listarConsumosCrianza(loteId),
      listarEventosSanitariosCrianza(loteId),
    ]);
    setPesadas(p);
    setMuertes(m);
    setVentas(v);
    setConsumos(c);
    setEventos(e);
    const r = await obtenerResumenCrianza(loteId);
    setResumen(r);
    const lt = await obtenerLoteCrianza(loteId);
    setLote(lt);
  }, [loteId]);
  ...
  const exportarLote = async (formato: FormatoExportacionAvicola) => {
    ...
    try {
      ...
      await exportarOperacionesLoteCrianza(
        {
  ...
  const guardarMuerte = async () => {
    try {
      ...
      await registrarMuerteCrianza(loteId, {
        fecha: fMuerte,
        cantidad: c,
  ...
  const guardarVenta = async () => {
    try {
      ...
      await registrarVentaCrianza(loteId, {
        fecha: fVenta,
        tipo: tipoVenta,
        cantidad: c,
  ...
  const guardarConsumo = async () => {
    try {
      ...
      await registrarConsumoCrianza(loteId, {
  ...
  const guardarSanidad = async () => {
    try {
      ...
      await registrarEventoSanitarioCrianza(loteId, {
        fecha: fSan,
        tipo: tipoSan.trim(),
  ...
  if (Number.isNaN(loteId)) {
    return (
      <Box sx={{ p: 3 }}>
        ...
        <Button sx={{ mt: 2 }} onClick={() => navigate('/avicola-crianza/lotes')}>
          Volver a lotes
        </Button>
      </Box>
    );
  ...
  return (
    <Box sx={{ p: 3 }}>
      <Box display="flex" flexWrap="wrap" alignItems="center" gap={2} mb={2}>
        <Button variant="outlined" onClick={() => navigate('/avicola-crianza/lotes')}>
          ← Lotes
        </Button>
        {lote && (
          ...
          {pestana === 0 && resumen && (
            <Paper sx={{ p: 2 }}>
              <Stack spacing={1}>
                ...
                <Typography>
                  <strong>Disponibles:</strong> {resumen.cantidadDisponible ?? '—'}
                </Typography>
                <Typography>
                  <strong>Suma muertes:</strong> {resumen.sumaMuertes ?? '—'}
                </Typography>
                <Typography>
                  <strong>Mortalidad (%):</strong>{' '}
                  {resumen.mortalidadPorcentaje != null ? Number(resumen.mortalidadPorcentaje).toFixed(2) : '—'}
                </Typography>
                <Typography>
                  <strong>Suma consumos (kg):</strong>{' '}
                  {resumen.sumaConsumos != null ? String(resumen.sumaConsumos) : '—'}
                </Typography>
                <Typography>
                  <strong>Peso ref. (kg):</strong>{' '}
                  {resumen.pesoPromedioReferencia != null ? String(resumen.pesoPromedioReferencia) : '—'}
                </Typography>
                <Typography>
      ...
      <Dialog open={dialogo === 'venta'} onClose={cerrarDialogos} fullWidth maxWidth="sm">
        ...
        <DialogContent>
          <Stack spacing={2} sx={{ mt: 1 }}>
            ...
            <TextField select label="Tipo" fullWidth value={tipoVenta} onChange={(ev) => setTipoVenta(ev.target.value as TipoVentaCrianza)}>
              {TIPOS_VENTA.map((t) => (
...
export default DetalleLoteCrianzaScreen;
