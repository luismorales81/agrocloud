...
import type { LecheriaEstablecimiento } from '../types';

const UbicacionEstablecimientosLecheriaScreen: React.FC = () => {
  const navigate = useNavigate();
  const [searchParams, setSearchParams] = useSearchParams();

  const [lista, setLista] = useState<LecheriaEstablecimiento[]>([]);
  ...
  const cargarLista = useCallback(async () => {
    try {
      setCargando(true);
      const e = await listarEstablecimientos();
      setLista(e);
      setError(null);
    } catch (err: unknown) {
  ...
  const guardar = async () => {
    ...
    try {
      ...
      setError(null);
      setExito(null);
      await actualizarEstablecimiento(Number(seleccionId), {
        nombre: seleccionado?.nombre ?? '',
        ubicacion: textoUbicacion.trim() || null,
        coordenadas: coordenadasTrim,
        capacidadAnimales: seleccionado?.capacidadAnimales ?? null,
  ...
  return (
    <Box sx={{ p: 3, maxWidth: 1100, mx: 'auto' }}>
      <Typography variant="h4" component="h1" gutterBottom>
        Ubicación en mapa — Lechería
      </Typography>
      <Typography variant="body2" color="text.secondary" paragraph>
        Definí el perímetro o punto GPS del establecimiento lechero para consultar clima y autocompletar
        temperatura y humedad en los registros de ordeñe.
      </Typography>
      ...
      {!cargando && (
        <Paper variant="outlined" sx={{ p: 2 }}>
          <FormControl fullWidth sx={{ mb: 2 }} size="small">
            <InputLabel id="sel-est-lecheria">Establecimiento</InputLabel>
            <Select
              labelId="sel-est-lecheria"
          ...
          {seleccionId !== '' && seleccionado && (
            <>
              ...
              <Box sx={{ mt: 2, display: 'flex', flexWrap: 'wrap', gap: 1 }}>
                ...
                <Button variant="outlined" onClick={() => navigate('/lecheria/establecimientos')}>
...
export default UbicacionEstablecimientosLecheriaScreen;
