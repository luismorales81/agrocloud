...
import type { FeedlotEstablecimiento } from '../types';

const UbicacionEstablecimientosFeedlotScreen: React.FC = () => {
  const navigate = useNavigate();
  const [searchParams, setSearchParams] = useSearchParams();

  const [lista, setLista] = useState<FeedlotEstablecimiento[]>([]);
  ...
  return (
    <Box sx={{ p: 3, maxWidth: 1100, mx: 'auto' }}>
      <Typography variant="h4" component="h1" gutterBottom>
        Ubicación en mapa — Feedlot
      </Typography>
      <Typography variant="body2" color="text.secondary" paragraph>
        Definí el perímetro o punto GPS del establecimiento feedlot para consultar clima y autocompletar temperatura y
        humedad en las lecturas de comedero.
      </Typography>
      ...
      {!cargando && (
        <Paper variant="outlined" sx={{ p: 2 }}>
          <FormControl fullWidth sx={{ mb: 2 }} size="small">
            <InputLabel id="sel-est-feedlot">Establecimiento</InputLabel>
            <Select
              labelId="sel-est-feedlot"
          ...
          {seleccionId !== '' && seleccionado && (
            <>
              ...
              <Box sx={{ mt: 2, display: 'flex', flexWrap: 'wrap', gap: 1 }}>
                ...
                <Button variant="outlined" onClick={() => navigate('/feedlot/establecimientos')}>
...
export default UbicacionEstablecimientosFeedlotScreen;
