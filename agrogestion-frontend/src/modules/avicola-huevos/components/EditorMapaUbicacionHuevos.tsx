...
import FieldWeatherButton from '../../../components/FieldWeatherButton';
...
/** Convierte anillo GeoJSON [lng,lat][] en {lat,lng}[] (misma lógica que cultivos / FieldsManagement). */
...
export interface EditorMapaUbicacionHuevosProps {
  /** Si el diálogo está visible (reinicia el mapa al abrir). */
  abierto: boolean;
...
/**
 * Mapa Google + dibujo de polígono (TerraDraw) y punto manual; mismo criterio de coordenadas que cultivos.
 * El perímetro se dibuja tras pulsar «Dibujar perímetro» (modo static al inicio); doble clic cierra el polígono.
 */
const EditorMapaUbicacionHuevos: React.FC<EditorMapaUbicacionHuevosProps> = ({
  abierto,
  coordenadasJson,
  ...
  useEffect(() => {
    ...
    const montarMapa = (centro: { lat: number; lng: number }, zoomInicial: number) => {
      ...
      drawInstance.on('finish', (id: string | number, context: { action?: string }) => {
        ...
        onCoordenadasJsonChange(JSON.stringify(coordenadas));
        setPerimetroDibujoActivo(false);
        try {
          if (poligonoLecturaRef.current) {
            poligonoLecturaRef.current.setMap(null);
  ...
  }, [abierto, listo, revisionMapa, destruirMapa, onCoordenadasJsonChange]);
  ...
  return (
    <div style={{ marginTop: 8 }}>
      <p style={{ fontSize: '0.85rem', color: '#4b5563', marginBottom: 8 }}>
        ...
        <strong>doble clic</strong> para cerrar el polígono (igual que en cultivos). También podés usar un solo punto
        con las coordenadas manuales abajo.
      </p>
      {error && (
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginTop: 10, alignItems: 'center' }}>
        ...
        {coordsList.length > 0 && <FieldWeatherButton fieldName="Establecimiento" coordinates={coordsList} />}
      </div>
...
export default EditorMapaUbicacionHuevos;
