...
import type { PorcinosDieta, PorcinosDietaFase, PorcinosDietaFaseSolicitud } from '../typesApiV2';
import {
  actualizarDieta,
  actualizarFaseDieta,
  crearDieta,
...
const DietasPorcinosScreen: React.FC = () => {
  ...
  const [editandoDieta, setEditandoDieta] = useState<PorcinosDieta | null>(null);
  ...
  const [editandoFase, setEditandoFase] = useState<PorcinosDietaFase | null>(null);
  const [nombreFase, setNombreFase] = useState('');
  const [diasDesde, setDiasDesde] = useState('');
  const [kgCabeza, setKgCabeza] = useState('');
  ...
  const abrirEditarDieta = (d: PorcinosDieta) => {
    setEditandoDieta(d);
  ...
  const abrirEditarFase = (dietaId: number, fase: PorcinosDietaFase) => {
    setDietaFaseId(dietaId);
    setEditandoFase(fase);
    setNombreFase(fase.nombreFase);
    setDiasDesde(String(fase.diasDesdeIngreso));
    setKgCabeza(String(fase.kgCabezaDia));
  ...
  const guardarFase = async () => {
    ...
    const kg = parseFloat(kgCabeza.replace(',', '.'));
    if (!nombreFase.trim() || Number.isNaN(dias) || Number.isNaN(kg) || kg <= 0) {
      setError('Complete nombre, días desde ingreso y kg/cabeza/día válidos.');
      return;
    }
    const cuerpo: PorcinosDietaFaseSolicitud = {
  ...
  return (
    <Box sx={{ p: { xs: 2, md: 3 }, maxWidth: 1100 }}>
      ...
      <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
        Definí fases por días desde el ingreso con kg por cabeza y día, vinculadas al inventario de insumos.
      </Typography>
                            ...
                            <Box sx={{ py: 1, pl: 6 }}>
                              {fases.length === 0 ? (
                                <Typography variant="body2" color="text.secondary">
                                  Sin fases. Agregá al menos una para asignarla a los lotes.
                                </Typography>
                              ) : (
                                <Table size="small">
                                  <TableHead>
                                    <TableRow>
                                      ...
                                      <TableCell align="right">kg/cab/día</TableCell>
                                  ...
                                  <TableBody>
                                    {fases.map((f) => (
                                      <TableRow key={f.id}>
                                        ...
                                        <TableCell align="right">{String(f.kgCabezaDia)}</TableCell>
      ...
      <Dialog open={dialogoDieta} onClose={() => setDialogoDieta(false)} fullWidth maxWidth="sm">
        ...
        <DialogContent>
          <Stack spacing={2} sx={{ mt: 1 }}>
            ...
            />
            <FormControlLabel
              control={<Switch checked={activoDieta} onChange={(ev) => setActivoDieta(ev.target.checked)} />}
              label="Activa"
            />
          </Stack>
        </DialogContent>
      ...
      <Dialog open={dialogoFase} onClose={() => setDialogoFase(false)} fullWidth maxWidth="sm">
        ...
        <DialogContent>
          <Stack spacing={2} sx={{ mt: 1 }}>
            ...
            />
            <TextField
              label="kg por cabeza y día"
              fullWidth
              value={kgCabeza}
              onChange={(ev) => setKgCabeza(ev.target.value)}
            />
            <Autocomplete<InsumoOpcionFeedlot>
              ...
              onChange={(value) => setInsumoFase(value != null && value !== '' ? Number(value) : '')}
              placeholder="Buscar insumo en inventario…"
              emptyMessage="No hay insumos de alimento. Cargalos en Insumos del módulo."
              maxHeight={280}
            />
          </Stack>
        </DialogContent>
...
export default DietasPorcinosScreen;
