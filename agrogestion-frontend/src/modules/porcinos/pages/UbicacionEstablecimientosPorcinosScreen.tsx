...
import type { PorcinosEstablecimiento } from '../typesApiV2';

const UbicacionEstablecimientosPorcinosScreen: React.FC = () => {
  const navigate = useNavigate();
  const [searchParams, setSearchParams] = useSearchParams();

  const [lista, setLista] = useState<PorcinosEstablecimiento[]>([]);
  ...
  return (
    <Box sx={{ p: 3, maxWidth: 1100, mx: 'auto' }}>
      <Typography variant="h4" component="h1" gutterBottom>
        Ubicación en mapa — Porcinos
      </Typography>
      <Typography variant="body2" color="text.secondary" paragraph>
        Definí el perímetro o punto GPS del establecimiento porcino para consultar clima y autocompletar
        temperatura y humedad en registros operativos.
      </Typography>
      ...
      {!cargando && (
        <Paper variant="outlined" sx={{ p: 2 }}>
          <FormControl fullWidth sx={{ mb: 2 }} size="small">
            <InputLabel id="sel-est-porcinos">Establecimiento</InputLabel>
            <Select
              labelId="sel-est-porcinos"
          ...
          {seleccionId !== '' && seleccionado && (
            <>
              ...
              <Box sx={{ mt: 2, display: 'flex', flexWrap: 'wrap', gap: 1 }}>
                ...
                <Button variant="outlined" onClick={() => navigate('/porcinos/establecimientos')}>
...
export default UbicacionEstablecimientosPorcinosScreen;
